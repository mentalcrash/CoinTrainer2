...
2. 직전 응답(`previous_indicators`)과 **최소 2개 이상 겹치지 않도록** 신선한 지표 조합을 구성합니다.
3. 매 생성 시 지표의 파라미터(기간, 임계값)를 항상 변형하여 전략의 다양성을 유지합니다.
4. **시장 상태 판단을 위한 명확한 지표(예: ADX, Bollinger Band squeeze 등)를 최소 1개 포함하여**,  
   현재 시장 상태를 "횡보", "추세", "변동성 확대" 중 하나로 정확히 분류하고 상태를 먼저 평가합니다.  
   - 시장 상태에 따라 전략의 진입 조건 강도를 동적으로 조정합니다.
   - 예시: ADX가 25 이상이고 상승 추세라면 적극적인 전략, ADX가 하락하거나 20 미만이면 보수적인 전략
5. API 호출 횟수를 최소화하기 위해 **데이터 호출 후 재사용 가능한 캐싱 로직**을 구현하며, API 호출 실패 시 최근 정상 데이터를 사용하도록 명시적 예외처리를 구현합니다.
6. 진입 조건 평가 시, 최근 ATR을 기반으로 한 **목표가 및 손절가 설정 로직을 반드시 포함하고**, 리스크-리워드 비율(1:1.5 이상)을 만족할 경우에만 매수 진입을 허용합니다.
7. **지표 계산 값, 시장 상태 평가 결과, 진입 조건 평가 과정 및 이유를 self.logger에 명확하고 세부적으로 로그로 기록합니다.**
...
"""
        ...
        user_prompt = f"""
요구사항:
- 클래스와 메서드 이름을 전략의 핵심을 명확하게 표현하도록 설정하십시오.
  (예: VolatilityBreakoutScalper, AdaptiveTrendMomentum 등)
- **should_buy()의 반환 값을 Tuple[bool, float, float]로 명확히 구현하며,**  
...
- API 호출 횟수를 최소화하고, 데이터 캐싱 및 예외처리(호출 실패 시 최근 정상 데이터 재활용) 로직을 상세히 명시하십시오.
...
- 함수 시그니처는 변경하지 않으며, 모든 주석은 한글로 작성하십시오.
