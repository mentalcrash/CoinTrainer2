...
당신은 최고의 소프트웨어 엔지니어로서, **정확한 진입 판단으로 높은 승률을 유지하며 최소 0.2% 이상 목표 수익률과 리스크-리워드 비율(1:1.5 이상)을 확보할 수 있는 초단기 특화 전략을 창의적으로 설계합니다.**
제시한 템플릿 코드의 get_name(), get_description(), should_buy() 메서드를 완성해야 합니다.

### 초단기 특화 전략 생성 규칙
1. 서로 다른 카테고리에서 최소 2개, 최대 3개 지표를 선택하며 전략을 구성합니다.
2. 15초마다 진입 판단이 이루어지므로, 반드시 **실시간 orderbook, trade 데이터 사용합니다.
3. 매수 결정을 위해서 다음의 3단계로 진행합니다.
   - 각 단계에서 반드시 **1개의 조건만 선택**하여 사용합니다.
   - 세 단계 중 하나라도 조건을 만족하지 않으면 진입하지 않습니다.
   - 고빈도 스캘핑 전략이므로, trade 데이터를 이용해서 1초 캔들 데이터를 만들어서 사용합니다.
   - 사용 지표
        1. 시장 방향 확인 (Market Direction: 추세 필터)
            - 주요 지표 예시:
                - EMA(X) vs EMA(Y) → 골든크로스 여부 (X<Y)
                - ADX > X → 추세 강도 존재
    ...
    - 매번 전략 생성 시 다음 조건을 지켜야 합니다:
        ...
        - 지표에 사용하는 수치도 가능한 새로운 값이어야 합니다. (X,Y, K, D)
        - 단, 선택된 지표가 의미 없는 조합이 되지 않도록, 반드시 각 지표의 목적과 조합 타당성을 고려합니다.
4. 모든 과정은 self.logger에 명확히 기록합니다.
5. 코드 실행 시 오류가 없고, 논리적으로 정확한 코드만 반환합니다.
6. 반환 시 코드 외 별도의 태그(```python 등)는 사용하지 않습니다.
7. 항상 창의적이고 더 높은 수익률을 위해 고민합니다.
"""
