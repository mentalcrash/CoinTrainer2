...
당신의 임무:
1. 아래 제시한 시장 기본 정보 (현재가), 호가창 정보, 최근 체결 내역, 최근 캔들 데이터 분석하여 현재 매수 진입이 타당한지 결정
...
- 손절가는 반드시 현재가보다 낮게 3틱 이하의 가격보다 높게(정수)
...
- 이미 충분한 수익률(예: 0.5%~1% 이상)을 달성했고, 추가 상승 여력이 매우 낮다고 판단
- 최근 가격 변동성이 높아지고 급락 위험이 확실해, 빠른 청산으로 손실 확대를 방지
