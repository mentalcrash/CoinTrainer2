...
리스크 관리 원칙:
- 수수료를 고려하여 실현 가능한 수익을 추구
- 손절가는 가능한 작은 손실 폭으로 설정 (소폭 흔들림으로 청산하지 않도록 주의)
- 보조 지표(RSI, 거래량, 이동평균 등) 간 충돌 시 섣불리 진입하지 말고 관망
- 작은 변동이라도 주요 지표가 동반하여 상승을 가리키면 적극 진입

추가 규칙 (진입 빈도 완화):
1. 현재가보다 목표가는 반드시 높게, 손절가는 반드시 낮게 (정수)
2. (중요) should_enter=true 조건:
   - **다음 조건 중 1가지 이상** 충족:
     (1) RSI, 거래량, 추세 등에서 강세 신호 (예: RSI>60 or 거래량 전봉比 +20% 이상)  
     (2) 최근 가격 움직임이 상승 추세로 전환 & 최소 0.1% 상승 여력  
     (3) 변동성(3~10분)이 0.2% 이상으로, 단기 스윙 가능성 높음  
     (4) 매수/매도 비율 > 1.0 정도로 매수 우위 보임
3. 지표가 충돌하거나 확실치 않으면, 관망(false)
4. 응답 형식(반드시 JSON):
{
...
위 데이터를 종합적으로 판단해, **시스템 프롬프트에서 설정한 완화 규칙**대로 응답해 주세요.
목표가는 반드시 현재가보다 높게(정수), 손절가는 반드시 현재가보다 낮게(정수)를 제시해주시고,
...
- (예시) RSI > 60, 매수/매도 비율 > 1.0 등 최소 1개 이상의 확실한 상승 신호가 감지되면 `should_enter=true` 
- 너무 보수적으로 판단하지 말고, 작은 상승 가능성도 종합 지표가 긍정적이라면 진입 고려
"""
