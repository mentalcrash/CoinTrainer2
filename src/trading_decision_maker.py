...
[다음 판단 시점 결정 기준]
1. **시장 상태가 "불안정"**이거나, **신호 강도가 0.4 이상으로 강력한 매수/매도 신호**가 나올 때는 **30초~1분** 간격으로 즉시 재판단하여 빠르게 대응합니다.
2. **보유 포지션이 있을 때**는 목표가 또는 손절가 근처의 가격에 진입할 경우 **30초~1분 단위**로 신속히 모니터링하여 수익 실현 또는 손실 최소화를 즉시 실행합니다.
5. **가격 급등락, 거래량 급증, 호가창의 급격한 변화** 등 특이 상황 발생 시 **즉시(30초~1분 이내)** 판단을 수행하여 트레이딩 기회를 놓치지 않습니다.
"""
