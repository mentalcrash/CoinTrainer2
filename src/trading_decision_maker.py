...
class TradingDecisionMaker:
    ...
    def _create_decision_prompt(
        ...
        Returns:
            프롬프트 문자열
        """
        prompt = f"""당신은 1시간 이내의 수익 실현을 목표로 빠르고 정확한 의사결정을 내리는 초단기 암호화폐 트레이딩 전문가입니다.

아래 제공된 {symbol}의 기술적 분석, 시장 흐름, 자산 정보를 바탕으로 지금 당장 취해야 할 **단기 매매 전략**을 판단해 주세요.

※ 당신의 판단은 “매수” 또는 “매도” 중 하나를 우선적으로 고려해야 하며,  
“관망”은 아래 조건을 모두 만족하는 경우에만 예외적으로 허용됩니다:
- 기술적 지표가 명확한 방향성을 제시하지 않음
- 거래량 및 변동성이 매우 낮고, 추세가 전혀 없음
- 뉴스 및 심리 데이터가 부족하거나 무의미함

❗주의: 관망을 선택할 경우 **즉각적인 수익 기회를 상실할 수 있으며**,  
이로 인한 **기회비용은 초단기 전략에서 매우 중요합니다**.  
모호하거나 책임 회피성 판단은 금지되며, 강력한 전략적 결정을 내려야 합니다.  
관망은 전략적 회피일 때만 허용되며, 그 이유를 정량적/정성적으로 반드시 설명해야 합니다.
...
📌 당신의 판단은 명확하고 구체적이어야 하며, 수익 기회와 리스크를 동시에 고려한  
책임 있는 트레이딩 전략을 수립하는 것을 목표로 합니다.
"""
